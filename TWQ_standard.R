...
df$mean_communication <- rowMeans(df[, c("Det er hyppig kommunikasjon innad i teamet", 
...
df$mean_coordination <- rowMeans(df[, c("Teamets deloppgaver er godt organisert", 
...
df$mean_mutual_support <- rowMeans(df[, c("Teammedlemmene utfyller og understøtter hverandres arbeid", 
...
df$mean_effort <- rowMeans(df[, c("Hvert teammedlem yter full innsats i teamarbeidet", 
...
df$mean_cohesion <- rowMeans(df[, c("Teamarbeidet er viktig for teamet", 
...
df$mean_balance_contrib <- rowMeans(df[, c("Teamet anerkjenner spesielle egenskaper (sterke og svake sider) hos de enkelte teammedlemmene", 
...
df$TWQ_Mean <- rowMeans(df[, c("mean_communication", 
                               "mean_coordination", 
                               "mean_mutual_support","mean_effort","mean_cohesion", "mean_balance_contrib" )], 
                        na.rm = TRUE)

df$TWQ_SD <- apply(df[, c("mean_communication", 
                          "mean_coordination", 
                          "mean_mutual_support", 
                          "mean_effort", 
                          "mean_cohesion", 
...
# Merge the real team compositions into the TWQ results
df_grouped <- df %>%
...
# Display the final TWQ results grouped by team composition
print(df_grouped)


# Group by gender and calculate mean for each TWQ factor, TWQ Mean, and TWQ SD
...
# Display the final TWQ results grouped by gender
print(df_grouped_by_gender)

# Group by gender and calculate mean for each TWQ factor, TWQ Mean, and TWQ SD along with team compositions
df_grouped_by_gender_composition <- df %>%
...
# Display the final TWQ results grouped by gender and team composition
print(df_grouped_by_gender_composition)
